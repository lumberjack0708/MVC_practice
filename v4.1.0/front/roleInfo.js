...
export default function roleInfo(){
    axios.get("../backend/index.php?action=getRoles")
    .then(res => {
        ...
        switch(response['status']){
            case 200:
                ...
                rows.forEach(element => {
                    str += `<tr>`;
                    str += `<td name='role_id'>` + element['role_id'] + `</td>`;
                    str += `<td>` + element['role_name'] + `</td>`;
                    str += `<td>` + element['created_at'] + `</td>`;
                    str += `<td style="text-align: center;"><button name='updateRole' class="custom-btn">修改</button>&ensp;<button name='deleteRole' class="custom-btn delete-btn">刪除</button></td>`;
                    str += `</tr>`;
                });
                str += `</tbody></table>`;
